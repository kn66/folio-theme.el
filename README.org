...
After ~folio-theme~ is available from MELPA, install it with
~M-x package-install RET folio-theme RET~ or declare it with ~use-package~:
...
The package autoload registers its installation directory in
~custom-theme-load-path~, so ~M-x load-theme~ and theme selection UIs can
discover ~folio~ without additional path configuration.

** Install from Git

Emacs 29 or later can install the latest revision directly from the repository:
...
- Interactive selection: distinct warm and soft-blue surfaces for ~region~
  and completion
- Terminal foregrounds: text-safe palette variants with at least 4.5:1
  contrast against ~bg-main~
...
- ~folio-theme-palette-partial~: base named colors for the theme
- ~folio-theme-palette-mappings-partial~: semantic role mappings in the Ef style
   for UI, headings, marks, modelines, and terminal colors
- ~folio-theme-custom-faces~: targeted package face overrides where inherited
